...
function deleteUnwantedData(array) {
    const desiredProperties = ["name", "image", "normalValue", "itemImage", "title", "value", "imageLink"];
...
function addItem(index) {
    ...
    itemDiv.onclick = () => {
        if (selectedsave == 1) {
            value1 = value1 - Number(datasave[index].normalValue);
        } else {
            value2 = value2 - Number(datasave[index].normalValue);
        }
...
Promise.all([
    fetch("https://petcatchersvalues.com/_functions/api/VALUES").then((response) => response.json())
])
    .then(([valueData]) => {
